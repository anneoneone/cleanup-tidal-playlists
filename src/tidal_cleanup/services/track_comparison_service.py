...
from ..models.models import Track, ComparisonResult
...
class TrackComparisonService:
    ...
    def normalize_track_name(self, track_name: str) -> str:
        ...
        if len(parts) == 2:
            ...
            artist = re.sub(
                r"(,| feat\.| & ).*", "", artist, flags=re.IGNORECASE
            ).strip().lower()
    ...
    def _clean_title(self, title: str) -> str:
        ...
        title = re.sub(
            r"(remix|edit|mix|version)", "", title, flags=re.IGNORECASE
        )
    ...
    def compare_track_sets(
        ...
        normalized_local = {
            self.normalize_track_name(track) for track in local_tracks
        }
        normalized_tidal = {
            self.normalize_track_name(track) for track in tidal_tracks
        }
    ...
    def find_fuzzy_matches(
        ...
        for track in unmatched_tracks:
            match_result = self.find_best_match(
                track, candidates_list, threshold
            )
            if match_result:
